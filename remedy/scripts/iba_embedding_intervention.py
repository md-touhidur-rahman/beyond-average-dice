...
class IBAInterpreter:
    ...
    def _train_bottleneck(self, text_t: torch.Tensor, image_t: torch.Tensor):
        ...
        if(not self.ensemble):
            ...
            optimizer = torch.optim.Adam(lr=self.lr, params=self.bottleneck.parameters())
            image_batch = image_t.expand(self.batch_size, -1, -1, -1)
            # Reset from previous run or modifications
            self.bottleneck.reset_alpha()
            # Train
            self.model.eval()
            for _ in tqdm(range(self.train_steps), desc="Training Bottleneck",
                            disable=not self.progbar):
                optimizer.zero_grad()
                image_output = self.model.get_image_features(image_t)
                text_outputs = self.model.get_text_features(text_t)
                text_outputs = text_outputs.mean(dim=0).unsqueeze(0)
                text_outputs = text_outputs / text_outputs.norm(dim=-1, keepdim=True)
                # print(text_outputs.shape)
                # print(text_outputs.expand(self.batch_size,-1).shape)
                loss_c, loss_f, loss_t = self.calc_loss(outputs=text_outputs.expand(self.batch_size,-1), labels=image_output)
                loss_t.backward()
                optimizer.step(closure=None)

        return loss_c, loss_f, loss_t 

    def vision_heatmap_embedding_target(self, text_target, image_t):
        """
        Vision IBA with an externally supplied projected text embedding.

        The vision bottleneck, optimizer, RNG-dependent bottleneck sampling,
        compression objective, fitting objective, and saliency extraction are
        otherwise identical to the original non-ensemble vision IBA path.
        """
        saliency, loss_c, loss_f, loss_t = \
            self._run_vision_training_embedding_target(
                text_target, image_t
            )

        saliency = torch.nansum(saliency, -1)[1:]
        dim = int(saliency.numel() ** 0.5)
        saliency = saliency.reshape(1, 1, dim, dim)
    ...
    def _run_vision_training_embedding_target(
        self,
        text_target,
        image_t
    ):
        ...
        loss_c, loss_f, loss_t = \
            self._train_bottleneck_embedding_target(
                text_target,
                image_t
            )
        ...
        return (
            self.bottleneck.buffer_capacity.mean(axis=0),
            loss_c,
            loss_f,
            loss_t
        )

    def _train_bottleneck_embedding_target(
        self,
        text_target,
        image_t
    ):
        # Match the original non-ensemble path exactly except that
        # projected text features are supplied rather than recomputed
        # from token IDs.
        target_batch = text_target.expand(
            self.batch_size, -1
        )

        image_batch = image_t.expand(
        ...
        for _ in tqdm(
            ...
            loss_c, loss_f, loss_t = self.calc_loss(
